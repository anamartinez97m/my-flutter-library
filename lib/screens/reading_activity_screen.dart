...
class _ReadingActivityScreenState extends State<ReadingActivityScreen>
  ...
  Future<void> _loadPastWinners() async {
    try {
      final db = await DatabaseHelper.instance.database;
      final repository = BookCompetitionRepository(db);
      final currentYear = DateTime.now().year;
      final winners = await repository.getPastYearsWinners(currentYear);
      if (mounted) {
        setState(() {
          _pastWinners = winners;
          _isLoadingPastWinners = false;
        });
      }
    } catch (e) {
      debugPrint('Error loading past winners: $e');
      if (mounted) {
        setState(() => _isLoadingPastWinners = false);
      }
    }
  }
              ...
              GestureDetector(
                onTap: () async {
                  await Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (_) => const NewPastYearsCompetitionScreen(),
                    ),
                  );
                  _loadPastWinners();
                },
