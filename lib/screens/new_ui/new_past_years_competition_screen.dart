...
class _NewPastYearsCompetitionScreenState
  ...
  double? _reigningChampionRating;
  ...
  Future<void> _loadData() async {
    try {
      ...
      final reigningChampionBook =
          reigningChampion == null
              ? null
              : await bookRepository.getBookById(reigningChampion.winnerBookId);
      final reigningChampionRating = reigningChampionBook?.myRating;

      if (mounted) {
        setState(() {
          _reigningChampion = reigningChampion;
          _reigningChampionRating =
              reigningChampionRating != null && reigningChampionRating > 0
                  ? reigningChampionRating
                  : null;
  ...
  List<BookCompetition> get _filteredPastWinners =>
      _pastWinners
          .where(
            (winner) =>
                winner.year != _reigningChampion?.year &&
                _filteredYears.contains(winner.year),
          )
          .toList();
