...
class EditBookScreen extends StatefulWidget {
  ...
  const EditBookScreen({super.key, required this.book, this.useNewUi = false});
...
class _EditBookScreenState extends State<EditBookScreen> {
  ...
  Future<void> _loadRatingFields() async {
    try {
      final db = await DatabaseHelper.instance.database;
      final ratingRepository = BookRatingFieldRepository(db);
      final fields = await ratingRepository.getRatingFieldsForBook(
        widget.book.bookId!,
      );
      setState(() {
  ...
  Future<void> _updateBook() async {
    ...
    try {
      ...
      final updatedBook = Book(
        ...
        description: _fetchedDescription ?? widget.book.description,
        coverUrl: _fetchedCoverUrl ?? widget.book.coverUrl,
        metadataSource: _fetchedMetadataSource ?? widget.book.metadataSource,
        metadataFetchedAt:
      ...
      // Save rating fields
      final ratingRepository = BookRatingFieldRepository(db);
      // Delete existing rating fields
      await ratingRepository.deleteAllRatingFieldsForBook(widget.book.bookId!);
      // Insert new rating fields
  ...
  ThemeData _v2Theme(BuildContext context) {
  ...
  Widget build(BuildContext context) {
    if (_isLoading) {
      return Scaffold(
        backgroundColor: widget.useNewUi ? _kBg : null,
        body: Center(
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          backgroundColor: widget.useNewUi ? _kBg : null,
          foregroundColor: widget.useNewUi ? _kPrimary : null,
          surfaceTintColor: widget.useNewUi ? Colors.transparent : null,
          elevation: widget.useNewUi ? 0 : null,
          title: Text(
            l10n.edit_book,
            style:
                widget.useNewUi
        ...
        bottomNavigationBar: widget.useNewUi ? _v2Footer(l10n) : null,
        body: Theme(
          data: widget.useNewUi ? _v2Theme(context) : Theme.of(context),
          child: SingleChildScrollView(
            padding: EdgeInsets.fromLTRB(
              20,
              widget.useNewUi ? 20 : 36,
              20,
              widget.useNewUi ? 20 : 50,
            ),
            child: Form(
              key: _formKey,
              child: Column(
                ...
                children: [
                  if (widget.useNewUi) _v2SectionHeading('Primary Info'),
                  ...
                  if (widget.useNewUi) ...[
                    _v2Divider(),
                    _v2SectionHeading('Book Details'),
                  ] else
                    const SizedBox(height: 16),
                  ...
                  ChipAutocompleteField(
                    ...
                    hintText:
                        AppLocalizations.of(context)!.search_or_add_author,
                    onChanged: (values) {
                      setState(() {
                  ...
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText:
                          AppLocalizations.of(context)!.write_your_thoughts,
                    ),
                    maxLines: 5,
                  ...
                  SizedBox(height: widget.useNewUi ? 16 : 50), // Bottom margin
                ],
              ),
            ),
          ),
